...
print("Staff Version with Vertical Histogram Extension with File Handling\n")
...
# function for unpacking individual progression data lists which consist of tuples onto the file
def unpacking_list(progression_outcome, progression_out_cat):
    for i in range(len(progression_outcome)):
        print(progression_out_cat, end=" ", file=prog_file)
        # https://www.geeksforgeeks.org/print-lists-in-python-4-different-ways/
        # referred the above webpage to find out how to remove the parentheses from a tuple when printed
        print(*progression_outcome[i], sep=", ", file=prog_file) 


# writing progression data to the file created
with open("progression outcomes.txt", "w") as prog_file:
...
# opening the file created to read the progression data
if os.path.exists("progression outcomes.txt"):
    with open("progression outcomes.txt", "r") as prog_file:
        print(prog_file.read())
